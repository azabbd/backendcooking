...
//$SearchGlob =  htmlspecialchars(filter_input(INPUT_POST, 'RechercheGlobale', FILTER_SANITIZE_STRING));
        ...
        if(preg_match("/^[A-Z\d]+$/i", $_POST['RechercheGlobale'])) //   ^/[A-Za-z]+/    
            { 
                ...
                //recherche cat
                /*$stmtRC = $mysqli->prepare("SELECT *FROM categories where nom LIKE ?   ;");
                
                $likeSearchGlob = "%" . $SearchGlob . "%";
                
                $stmtRC->bind_param('s',$likeSearchGlob );  // Bind "$idRecetteGet" to ?. //<input type=button onClick="location.href='index.html'" value='click here'>
                $stmtRC->execute();  
                $result = $stmtRC->get_result();
                ...
                if ($stmtRecettesCat->num_rows > 0) 
                    ...
                    echo '<div class="title  my-2     " style="color:#9D0461; text-align: left;"><h4>  Recherche "'.$SearchGlob.'" suivant une catégorie contenant '.$SearchGlob .'</h4><br></div>';
                    while($row = $stmtRecettesCat->fetch_assoc()) 
                        {
                            ...
                            <h2 style="  text-align: center;"> <a href="recettedetails.php?idRecette='.$row['idRecette'].'" style="color: #9D0461;">'.$row['titre'].'</a></h2> 
                            </div>';
                        }
                    echo' </div>';
    ...
    //recherche dans membre
                /*$stmtR = $mysqli->prepare("SELECT * FROM membres   where statut LIKE ? OR prenom LIKE ?  OR nom LIKE ?");
                            
                $likeSearchGlobR = "%" . $SearchGlob . "%";
                //echo  $likeSearchGlob .'<br>';
                $stmtR->bind_param('sss',$likeSearchGlobR, $likeSearchGlobR, $likeSearchGlobR);  // Bind "$idRecetteGet" to ?. //<input type=button onClick="location.href='index.html'" value='click here'>
                //$stmtR->bind_param(2,$likeSearchGlobR, PDO::PARAM_STR);
                $stmtR->execute();  
                $result = $stmtR->get_result();
                $pointRech=0;
                $countrecNews=0;
                if ($result->num_rows > 0) 
                {
                $fetchRec=$result->fetch_assoc()["idMembre"];
                $stmtRecettesCat= $mysqli->query("SELECT * FROM recettes where membre=$fetchRec  ;");
                
                if ($stmtRecettesCat->num_rows > 0) 
                    {
                    echo ' <div class="row  mx-auto py-2  my-2  ">';
                    echo '  <div class="title  my-2     " style="color:#9D0461; text-align: left;">
                                <h4>  Recherche "'.$SearchGlob.'" suivant un membre contenant '.$SearchGlob .'</h4><br>
                            </div>';
        ...
        { 
            echo '<script language="javascript"> alert("Please enter a search query, redirecting to homepage on click")
